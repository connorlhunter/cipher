...
pub const PREVIOUS_PROTOCOL_VERSION: u16 = 0;
...
pub fn desktop_status(protocol_version: Option<u16>) -> Result<DesktopStatus, IpcError> {
    let version = protocol_version.unwrap_or(PREVIOUS_PROTOCOL_VERSION);
...
mod tests {
    ...
    fn accepts_current_and_previous_protocol_versions() {
        ...
        assert!(!supports_protocol_version(CURRENT_PROTOCOL_VERSION + 1));
    }

    #[test]
    fn status_view_is_bounded_and_safe() {
        let status = desktop_status(Some(CURRENT_PROTOCOL_VERSION)).unwrap();
        assert!(status.message.len() <= MAX_STATUS_MESSAGE_LENGTH);
    }

    #[test]
    fn rejects_an_unsupported_protocol_version() {
        let error = desktop_status(Some(CURRENT_PROTOCOL_VERSION + 1)).unwrap_err();
    ...
    fn shared_status_fixtures_cover_current_and_previous_versions() {
        for fixture in [
            ...
            let version = fixture["protocolVersion"].as_u64().unwrap() as u16;
